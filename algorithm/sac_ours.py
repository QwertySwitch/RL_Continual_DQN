...
def train_sacours(agent, env, hyper_params):
    #env.reset()
...
class SACOursAgent:
    def __init__(self,
        ...
        self.previous_policy_network = CateoricalPolicy(action_space.n).to(device)
    ...
    def ar_update_target(self):
        online = copy.deepcopy(self.online_critic.state_dict())
        target = copy.deepcopy(self.target_critic.state_dict())
        for key in online.keys():
            target[key] = self.ema_decay * target[key] + (1-self.ema_decay) * online[key]
    ...
    def learn(self):
        ...
        if self.first==False:
            contrastive_loss = self.calculate_contrastive_loss(states)
            policy_loss = policy_loss + contrastive_loss
        entropy_loss = self.calc_entropy_loss(entropies, weights)
        
        
        self.previous_policy_network.load_state_dict(self.policy.state_dict())
    ...
    def calculate_contrastive_loss(self, states):
        
        device = self.device
        
        prev_feature = self.previous_policy_network.sample(states, True)
        curr_feature = self.policy.sample(states, True)
        q1, q2 = self.online_critic(states, True)
        q = torch.min(q1, q2)
        
        posi = self.cos(q1, curr_feature)
